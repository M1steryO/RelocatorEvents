...
func (m *UserInfo) validate(all bool) error {
	...
	if all {
		switch v := interface{}(m.GetTelegramId()).(type) {
		case interface{ ValidateAll() error }:
			if err := v.ValidateAll(); err != nil {
	...
	} else if v, ok := interface{}(m.GetTelegramId()).(interface{ Validate() error }); ok {
		if err := v.Validate(); err != nil {
	...
	for idx, item := range m.GetInterests() {
		_, _ = idx, item
		...
		if all {
			...
			case interface{ ValidateAll() error }:
				if err := v.ValidateAll(); err != nil {
					errors = append(errors, UserInfoValidationError{
						field:  fmt.Sprintf("Interests[%v]", idx),
						reason: "embedded message failed validation",
						cause:  err,
					})
				}
