...
export const HomePage = () => {
    ...
    const fetchEvents = async (isInitial: boolean) => {
        ...
        try {
            const params: GetListRequest = {
                ...
                limit: PAGE_LIMIT,
                last_id: isInitial ? undefined : lastId ?? undefined,
                ...appliedFilters,
            };
            ...
            setEvents((prev) => (isInitial ? displayEvents : [...prev, ...displayEvents]));

            if (response.filters) {
                setAvailableFilters(response.filters);
            }

            const newLastId = response.data.length > 0 ? response.data[response.data.length - 1].id : null;
            setLastId(newLastId);
    ...
    }, [hasMore, isLoading, isLoadingMore, lastId, debouncedSearchQuery, currentSort, appliedFilters]);
